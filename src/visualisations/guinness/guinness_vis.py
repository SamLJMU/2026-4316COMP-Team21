    ...
    if selected_countries:
        ...
        for country in selected_countries:
            ...
            if not country_df.empty:
                ...
                # Plot Monthly Average Humidity on bottom chart (ax2)
                if 'humidity' in avg_df.columns:
                    hum_df = country_df.dropna(subset=['humidity'])
                    if not hum_df.empty:
                        ax2.plot(hum_df['month_start'], hum_df['humidity'], marker='o', label=country, linewidth=1.5)
                    else:
