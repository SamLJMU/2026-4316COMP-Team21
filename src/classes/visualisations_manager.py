        ...
        with open("visualisation_config.json") as f:
            ...
            for idx, v in enumerate(json_visualisations, start_index):
                ...
                if v["key"] not in function_map:
                    print_error(
                        f"No mapping for '{v['key']}' in function_map, ensure keys match in visualisation_config.json and function_mapping.py"
                    )
                    raise KeyError(
                        f"No function mapped to this key in function_mapping.py. Key: `{v['key']}`"
                    )
